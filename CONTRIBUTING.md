...
- **Label the PR with a target label** — `target/vCurrent` for the line in flight, `target/vNext` for work held for the next yearly major, `target/v10` for legacy `support/v10` maintenance. (Older PRs use the previous absolute scheme, `target/2026` and friends; check `gh label list` for what the repo currently offers.) **Breaking changes are batched to the yearly major cut**: they may land on `experimental` only (never `main`, never a `release/YYYY` production train), are held for next year's `YYYY+1.0.0`, and additionally get a `breaking-change` label, a `⚠️ Breaking change` callout, and a `CHANGELOG.md` entry under the next-major `[Unreleased]` heading. Surface that isn't ready to commit to can ship behind `[Experimental("FALLOUT0xx")]` instead of being held back. The full procedure — including what counts as breaking — is in [`.agents/skills/creating-a-pr/SKILL.md`](.agents/skills/creating-a-pr/SKILL.md).
